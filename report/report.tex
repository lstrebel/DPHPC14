...
%		The set of vertices at the same minimal distance to the root is refered to as a level.
	...
	\section{Related work} \label{sec:rewo} % Roughly column. + ~8 references.
		...
		For the final proposed technique we go the way of avoiding atomic operations and synchronization within levels at all.
	...
	\section{Design and implementation}\label{sec:deim} % Roughly page + half a column. + 1 code snippet
		...
		\subsection{Optimistic BFS algorithm design}
			...
			Our approach utilizes $3 \cdot number\_of\_the\_vertices$ bytes of memory allocated in 3 equal chunks:
			\begin{enumerate}
	...
	\section{Experiments}\label{sec:expe} % Roughly two pages - half a column. + 4 plots
		...
		For each pair of graph and platform the complete distance map was built for 15 root vertices equally distributed on the entire graph using each of the BFS alternatives.
		...
		Improved naive implementation, which attempts to get rid of the critical section by using an atomic CAS operation, to check whether a vertex was visited and to set the correct distance. 
