...
\hyphenation{atte-mpting}
\hyphenation{avo-iding}
\hyphenation{co-mputer}
\hyphenation{do-esn}
\hyphenation{en-abl-ed}
\hyphenation{no-mi-nal}
\hyphenation{nu-mber}
\hyphenation{pe-ak}
\hyphenation{phy-sical}
\hyphenation{There}
...
%		The set of vertices at the same minimal distance to the root is refered to as a level.
	...
	\section{Related work} \label{sec:rewo} % Roughly column. + ~8 references.
		...
		It is inherently memory intensive and has poor spatial locality which introduces significant performance loss on todays computer systems with the growing gap between CPU and memory performance and increasing memory latencies.
	...
	\section{Design and implementation}\label{sec:deim} % Roughly page + half a column. + 1 code snippet
		...
		\subsection{Possible directions of performance boosting}
			...
			Careful cache-aw-are algorithm design can introduce significant performance benefits on the today's SMA computer systems.
		...
		\subsection{Optimistic BFS algorithm design}
			...
			Our approach utilizes memory allocated in 3 chunks, each of which with size $number\_of\_the\_vertices$ bytes:
			\begin{enumerate}
	...
	\section{Experiments}\label{sec:expe} % Roughly two pages - half a column. + 4 plots
		We present experimental results to demonstrate the performance gains that can be realized with use of Optimistic BFS.
		There are three experimental testbeds used in the experiments. 
		In all cases, scalability measurements were limited by the number of physical processing elements present in the test environment (including processing elements delivered by Hy-per-Threading technology).
		...
		To account for this, the real world graphs and specific synthetic graphs provided by the University of Florida Sparse Matrix Collection \cite{SparseMatrixCollection} was used during experiments.
		...
		Each experiment was repeated for diffent number of parallel threads in the rage from 1 and limiting by the number of physical processing elements provided by testbed.
		Minimal, maximal and average processing time was computed.
		
		\subsection{Alternative parallel BFS methods}\label{sec:approaches}
		Many different avlternative BFS methods was considered during a study and evaluated during experiments.
		...
		Improved naive implementation attempting to get rid of the critical section by using an atomic CAS operation, to check whether a vertex was visited and to set the correct distance. 
...
%		\end{figure}
		...
		Reasons for this might be the small number of edges in this graph limiting the potential for parallel execution to a point where the cost of doing any synchronization or the additional work created to circumvent synchronization on this system is too great.
		...
		The main result from this experiment is the observation that any version that relies on OMP critical section, even if only to manipulate the used data structure, does not perform in a useful way.
		As a minor detail, the jump in the non-level-synchronous version in this plot or more clearly the jump for the Optimistic modified version in Fig. \ref{fig:eulerbig} between 12 and 13 threads can be explained by the crossing the barier between physical cores and logical cores provided by hyperthreading.
