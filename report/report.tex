...
%		Especially the small-world property is worth mentioning since it provides a reasonable assumption that a given node in a network has a large enough neighbourhood so that a parallel BFS can be useful.	
	...
	\section{Design and implementation}\label{sec:deim} % Roughtly page + half a column. + 1 code snippet
		...
		\subsection{Optimistic BFS algorithm design}
			...
			After the end of each level processing the last two flag arrays exchange their roles by simple and efficient pointer swapping, and next level nodes array is resetting.
			...
			The most important point in this algorithm that it doesn't use any synchronization in the body of the parallel for loop.
			As a result it prone to the race conditions when two or more concurrently processing current nodes leads to the handling of the same node which is neighbour node of both.
			But in fact, this is a false race condition, because firstly, algorithm for each iteration algorithm separates from each other data (and memory storing them) which will be read and which will be updated, and secondly it provide a guarantee that the same memory locations will be updated by the same values.
			...
			On the other hand such false race conditions leads to the possible useless work done due to the concurrent processing of the same neighbour vertices.
			Due to this we call this approach optimistic, because each iteration is made in hope that its result will be valuable for the overall progress.  
...
%		\end{figure}
	...
	\section{Summary and Conclusion}\label{sec:suco} % Roughly half a column.
		...
		In our experimental evaluation, we found that the level of optimality of BFS algorithms heavily depends on characteristics of the target environment, including CPU architecture, operating system and compiler used, and on characteristics of the target graph.
