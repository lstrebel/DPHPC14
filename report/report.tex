...
%	Especially the small-world property is worth mentioning since it provides a reasonable assumption that a given node in a network has a large enough neighbourhood so that a parallel BFS can be useful.
	...
	\section{Experiments}\label{sec:expe} % Roughtly two pages - half a column. + 4 plots
		...
		There are three experimental testbeds was used during experiments. 
		...
		\begin{itemize}
			...
			\item \textbf{Einstein}. Intel Xeon Phi accelerator with 61 cores. Intel ICC compiler, with -O2 flag.
			%TODO: What version of accelerator, Linux kernel and ICC compiler was used -}
	...
	\section{Discussion}\label{sec:disc} % Roughtly column. + ~1-2 references + 1 table
		...
		We would like to note that the study can't be considered as a complete, and additional experiments and analysis need to be conducted to achieve clear understanding of the all properties of the proposed algorithm.  
