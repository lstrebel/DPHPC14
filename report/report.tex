...
%		Especially the small-world property is worth mentioning since it provides a reasonable assumption that a given vertex in a network has a large enough neighbourhood so that a parallel BFS can be useful.	
...
%		In addition to this, we have compared a number of different approaches based on different synchronization primitives and techniques and load balancing strategies. %TODO
	...
	\section{Design and implementation}\label{sec:deim} % Roughly page + half a column. + 1 code snippet
		...
		\subsection{Optimistic BFS algorithm design}
			...
			After the end of processing each level, the last two flag arrays exchange their roles by simple and efficient pointer swapping, and next level vertices array is resetted.
			...
			A byte per vertex memory allocation scheme was chosen instead of classical bit masks because it allows to simplify memory access patterns and eliminate synchronization that would be required otherwise.
			...
			As a result, consistency of the data is preserved during entire graph processing time and it guarantees consistency of the results even without the use of explicit synchronization.
			On the other hand, such false race conditions lead to possibly useless work being done due to the concurrent processing of the same neighbour vertices.
