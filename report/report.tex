...
\begin{document}
	...
	\begin{abstract} % Roughly half a column.
		...
		Breadth-first search (BFS) is a crucial graph traversal algorithm with many industrial applications.
		In this paper, we will look into several implementations of parallel BFS algorithms and compare their performance for different shared-memory architecture systems and multiple graphs.
		The most promising algorithm is one to avoid explicit synchronization, which seems to be the main culprit for performance issues.
		Even though the finally proposed algorithm performed better than the other options considered, the underlying architecture and the target graph have a big influence on its performance, which leads to the conclusion that it isn't possible to find one optimal parallel BFS implementation without a specific architecture and a target graph in mind.
	\end{abstract}
...
%		Our experiments and analysis of its results highlight that the efficiency of BFS algorithm variants heavily depend on the properties of the underlying environment (hardware platform, operating system, compiler), as well as on the properties of the graph to which it is applied. % -> take these two sentences out, the introduction doesn't have to give the results
